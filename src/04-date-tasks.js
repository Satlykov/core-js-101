...
function timeSpanToString(startDate, endDate) {
  const timeDiff = new Date(Math.abs(startDate - endDate));
  return `${timeDiff.toUTCString().slice(17, 25)}.${timeDiff
    .getUTCMilliseconds()
    .toString()
    .padStart(3, '0')}`;
}
...
function angleBetweenClockHands(date) {
  ...
  if (ans / 180 > 1) {
    return Math.abs(((ans / 180) - 2) * Math.PI);
  }
  return Math.abs((ans / 180) * Math.PI);
}
